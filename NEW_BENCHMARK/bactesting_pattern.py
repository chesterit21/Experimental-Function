...
def process_game_data(df, game_code):
    ...
    # REFACTOR: Reset index untuk bekerja dengan posisi integer dan menghindari error
    #           ketika 'Periode' duplikat menyebabkan get_loc() mengembalikan slice.
    df_with_pos = df.reset_index()
    ...
    for pattern_name, pattern_value in patterns.items():
        # Mencari di data historis (semua kecuali baris terakhir dari df_with_pos)
        historical_df = df_with_pos.iloc[:-1]
        matches = historical_df[historical_df['LogResult'].str.contains(pattern_value, na=False)]
        
        for loc, matched_row in matches.iterrows():
            # 'loc' sekarang adalah posisi integer yang aman untuk perbandingan.
            
            # Ambil LogResult dari data sebelum, data yang cocok, dan data sesudah.
            if loc > 0:
                initial_found_results.add(df_with_pos.iloc[loc - 1]['LogResult'])
            initial_found_results.add(matched_row['LogResult'])
            if loc < len(df_with_pos) - 1:
                initial_found_results.add(df_with_pos.iloc[loc + 1]['LogResult'])
    ...
    # File 2: Menyimpan data murni (hanya angka kandidat) ke dalam file JSON.
    os.makedirs(PURE_DATA_DIR, exist_ok=True)
    pure_json_filepath = os.path.join(PURE_DATA_DIR, f"{game_code}_{latest_periode}.json")
    pure_data_list = sorted(final_candidates.keys())
    with open(pure_json_filepath, 'w') as f_json:
        json.dump(pure_data_list, f_json)
